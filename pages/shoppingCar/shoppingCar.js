Page({  /**   * 页面的初始数据   */
  data: {
    ...
    cartsdata: [{
      ...
      selected: false,
      goodsinfo: [{
        goodsname: 'Fei家丝绸100cm大方巾白搭旅游保暖围巾女百搭秋季',
        specification: '规格',
        price: '100.22',
        num: '1',
        selected: false
    ...
    for (var i = 0; i < cartsdata.length; i++) {
      ...
      for (var a = 0; a < goodsinfo.length; a++) {
        if (goodsinfo[a].selected) {
          ...
          let num = parseInt(goodsinfo[a].num)
          //防止num为字符 *1或parseInt Number         
          allprices += price * num
          allnum += num
        }
      }
  ...
  storeselected: function (e) {
    ...
    //当前店铺下标    
    var thisstoredata = cartsdata[index].goodsinfo
    //当前店铺商品数据    
    //改变当前店铺状态    
  ...
  goodsselected: function (e) {
    ...
    let cai = cartsdata[idx].goodsinfo;
    //当前商品的店铺data.goodsinfo    
    let curt = cai[index];
    //当前商品数组   
    if (curt.selected) {
      cartsdata[idx].goodsinfo[index].selected = false
      //点击后当前店铺下当前商品的状态
      cartsdata[idx].selected = false
    } else {
      cartsdata[idx].goodsinfo[index].selected = true
      //点击后当前店铺下当前商品的状态    
  ...
  addCount: function (e) {
    ...
    //当前商品的数量    
    num++;
    cartsdata[idx].goodsinfo[index].num = num
    //点击后当前店铺下当前商品的数量    
    this.setData({
      cartsdata: cartsdata
      //店铺下商品的数量    
